...
(function () {
  ...
  effectLevelPin.addEventListener('dragstart', function onEffectLevelPinDragStart(evt) {
    evt.preventDefault();
  });

  window.initializeEffects = function (effectElem, applyEffect) {
    effectControls.addEventListener('change', function onEffectControlsChange(evt) {
      if (evt.target.name !== 'effect') {
        return;
      }

      applyEffect(effectElem, evt.target.value, currentEffect);
